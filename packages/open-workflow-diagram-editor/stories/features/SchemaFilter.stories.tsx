...
} from "../../src/core/schema/filterSchemaByDefinition";
...
const TASK_DEFINITIONS: WorkflowDefinitionName[] = [
  "callTask",
  "doTask",
  "emitTask",
  "forTask",
...
function SchemaFilterStory() {
  const [taskDef, setTaskDef] = useState<WorkflowDefinitionName>("callTask");
  ...
  const handleTaskChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const newTask = e.target.value as WorkflowDefinitionName;
    setTaskDef(newTask);
    setSubType("");
    setSchemaJson("");
    setError("");
  ...
  const handleGetSchema = () => {
    ...
    try {
      ...
      const target = (subType || taskDef) as WorkflowDefinitionName;
...
The function ships inside the \`@openworkflowspec/diagram-editor\` package.
\`allof-merge\` is a declared dependency and bundled automatically —
...
\`\`\`ts
import { getSchemaForDefinition } from "@openworkflowspec/diagram-editor";
...
import {
  getSchemaForDefinition,
  getReferencedDefinitions,
} from "@openworkflowspec/diagram-editor";
...
// => ["asyncApiOutboundMessage", "asyncApiServer", "asyncApiSubscription",
//     "endpoint", "externalResource", "referenceableAuthenticationPolicy",
//     "runtimeExpression", "duration"]
...
\`WorkflowDefinitionName\` is a TypeScript union of every key in \`$defs\` of
\`workflow.json\`. Passing an unknown name is a **compile-time error**.

### Task definitions

| Name | Properties after merge |
|---|---|
| \`callTask\` | \`oneOf\` of 7 variants (AsyncAPI, gRPC, HTTP, OpenAPI, A2A, MCP, custom); each variant has \`taskBase\` fields inlined |
| \`doTask\` | \`if\`, \`input\`, \`output\`, \`export\`, \`timeout\`, \`then\`, \`metadata\`, \`do\` |
| \`emitTask\` | \`if\`, \`input\`, \`output\`, \`export\`, \`timeout\`, \`then\`, \`metadata\`, \`emit\` |
| \`forTask\` | \`if\`, \`input\`, \`output\`, \`export\`, \`timeout\`, \`then\`, \`metadata\`, \`for\`, \`while\`, \`do\` |
| \`forkTask\` | \`if\`, \`input\`, \`output\`, \`export\`, \`timeout\`, \`then\`, \`metadata\`, \`fork\` |
| \`listenTask\` | \`if\`, \`input\`, \`output\`, \`export\`, \`timeout\`, \`then\`, \`metadata\`, \`listen\`, \`foreach\` |
| \`raiseTask\` | \`if\`, \`input\`, \`output\`, \`export\`, \`timeout\`, \`then\`, \`metadata\`, \`raise\` |
| \`runTask\` | \`if\`, \`input\`, \`output\`, \`export\`, \`timeout\`, \`then\`, \`metadata\`, \`run\` |
| \`setTask\` | \`if\`, \`input\`, \`output\`, \`export\`, \`timeout\`, \`then\`, \`metadata\`, \`set\` |
| \`switchTask\` | \`if\`, \`input\`, \`output\`, \`export\`, \`timeout\`, \`then\`, \`metadata\`, \`switch\` |
| \`tryTask\` | \`if\`, \`input\`, \`output\`, \`export\`, \`timeout\`, \`then\`, \`metadata\`, \`try\`, \`catch\` |
| \`waitTask\` | \`if\`, \`input\`, \`output\`, \`export\`, \`timeout\`, \`then\`, \`metadata\`, \`wait\` |

### Other definitions

All other names in \`$defs\` are also valid. Common ones:

| Name | Description |
|---|---|
| \`task\` | Union of all 12 task types |
| \`taskBase\` | Shared base fields inherited by every task |
| \`retryPolicy\` | Retry back-off and limit configuration |
| \`authenticationPolicy\` | Auth policy union (basic, bearer, digest, OAuth2, OIDC) |
| \`error\` | Error object shape |
| \`endpoint\` | Endpoint / URI-template shape |
| \`timeout\` | Timeout duration definition |
| \`duration\` | Inline, expression, or literal duration |
| \`eventConsumptionStrategy\` | \`all\` / \`any\` / \`one\` event consumption shape |

---

## How it works

1. **Import** — \`workflow.json\` is imported as a static ES module at build
   time (Vite handles JSON imports natively — no file I/O at runtime).
...
\`\`\`ts
/** All keys available in the workflow schema $defs block. */
export type WorkflowDefinitionName = keyof typeof workflowSchema["$defs"];

/** A fully resolved, self-contained JSON schema object — no allOf remaining. */
 ...
 * excluding structural scaffolding (taskBase, taskList).
 */
export declare function getReferencedDefinitions(
  definitionName: WorkflowDefinitionName,
): string[];

/**
 * Resolves a named definition from workflow.json into a fully formed schema.
 * The merge result is cached — repeated calls for different definitions
 * are O(1) after the first invocation.
 * @throws {Error} if the definition name is not found.
 */
export declare function getSchemaForDefinition(
  definitionName: WorkflowDefinitionName,
): DereferencedSchema;
\`\`\`
`,
